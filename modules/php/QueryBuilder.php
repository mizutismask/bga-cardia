...
class QueryBuilder extends \APP_DbObject {
  ...
  public function __construct($table, $cast = null, $primary = 'card_id', $log = false) {
    $this->table = $table;
    $this->cast = $cast;
  ...
  public function values($rows = []) {
    ...
    foreach ($rows as $row) {
      $rowValues = [];
      foreach ($row as $val) {
        $rowValues[] = $val === null ? 'NULL' : "'" . $this->escapeStringForDB($val) . "'";
      }
  ...
  private function protect($arg) {
    return is_string($arg) ? "'" . $this->escapeStringForDB($arg) . "'" : $arg;
  }
